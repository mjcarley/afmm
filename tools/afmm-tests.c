...
{
  gdouble G[1024], Gc[1024], err, rr[3], zz[3], zero, chi ;
  gint n, str ;
  ...
  fprintf(stderr, "chi = %lg\n", chi) ;
  zero = 0 ;
  afmm_laplace_gfunc(N, r, r1, z, Gc, str) ;
  /* afmm_laplace_gfunc_vec(N, rr, 1, zz, 1, &r1, 0, &zero, 0, 3, Gc, str) ; */

  afmm_laplace_gfunc_comp(N, rr[0], r1, zz[0], G) ;
...

{
  ...
  afmm_tree_add_sources(tree, rz1, 2*sizeof(gdouble), nrz1, FALSE) ;
...

{
  ...
  afmm_laplace_s2l_matrices(N, L, dG, nd, LS, LP, 1.0,
...
static void vector_test(gdouble r, gdouble r1, gdouble z,	gint N)

{
  gdouble G[1024], Gv[8192], err, ri[32], r1i[32], zi[32], z1i[32] ;
  ...
  afmm_laplace_gfunc_vec(N, ri, 1, zi, 1, r1i, 1, z1i, 1, ng, Gv, sdist) ;
  
  /* afmm_laplace_gfunc_comp(N, r, r1, z, G) ; */

  err = 0 ;
  for ( i = 0 ; i < ng ; i ++ ) {
    afmm_laplace_gfunc(N, ri[i], r1i[i], zi[i]-z1i[i], G, str) ;
    for ( n = 0 ; n <= N ; n ++ ) {
      fprintf(stdout, "%d %lg %lg (%lg)\n", n, G[n], Gv[n*sdist+i],
