...

{
  ...
  *rz = (gfloat *)fftw_malloc((*np)*(*pstr)*sizeof(gfloat)) ;

  if ( *ns != 0 ) {
#ifdef AFMM_INTERLEAVE_SOURCE_DATA
    *src = &((*rz)[2]) ;
#else /*AFMM_INTERLEAVE_SOURCE_DATA*/
    *src = (gfloat *)fftw_malloc((*np)*(*sdist)*sizeof(gfloat)) ;
#endif /*AFMM_INTERLEAVE_SOURCE_DATA*/    
  } else {
    src = NULL ;
  }
  ...
  for ( i = 0 ; i < *np ; i ++ ) {
    ...
    if ( src != NULL ) {
      for ( j = 0 ; j < *ns ; j ++ ) {
	for ( k = 0 ; k <= *N ; k ++ ) {
	  fscanf(f, "%g", &((*src)[i*(*sdist) + j*(*sstr) + 2*k + 0])) ;
	  fscanf(f, "%g", &((*src)[i*(*sdist) + j*(*sstr) + 2*k + 1])) ;
	}
      }
    }
  }
...

{
  ...
  if ( sort_sources ) {
    afmm_sort_point_list_f(rz1, pstr*sizeof(gfloat), nrz1,
			       rmin, rmax, zmin, zmax) ;
  }
  ...
  afmm_tree_add_sources_f(tree, rz1, pstr*sizeof(gfloat), nrz1,
			      sort_sources) ;
  afmm_tree_add_field_f(tree, rz, fpstr*sizeof(gfloat), nfld, FALSE) ;
  ...
  afmm_tree_leaf_expansions_f(tree, C, cdist, cstr, TRUE) ;
